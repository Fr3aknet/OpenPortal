...
if [ "$(id -u)" != "0" ]; then
	echo "Please ececute this script as root"
	exit 1
fi
...
###	nip=`cat hosts |tail -$next|head -1|cut -d' ' -f2`
	...
	#Solucio a un problema en ip's acavades en tres digits
	fsolu=6
	solu=`cat hosts |tail -$next|head -1|cut -d' ' -f2|cut -d'.' -f4`
	if [ $solu -ge 100 ];then
		fsolu=`expr $fsolu - 1`
	fi

	nmac=`cat hosts |tail -$next|head -1|cut -d' ' -f$fsolu`
	echo $nmac
